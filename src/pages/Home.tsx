...
const ProductFeaturedCard = ({ title, price, desc, image, lang }: { title: string, price: string, desc: string, image: string, lang: string }) => {
  return (
    <motion.div
      whileHover={{ y: -5 }}
    ...
    >
      <div className="w-24 h-24 rounded-2xl overflow-hidden flex-shrink-0 border border-brand-green/5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <Link to="/products" className={cn("text-[10px] font-black text-white bg-brand-orange px-4 py-2 rounded-lg inline-block uppercase tracking-wider hover:bg-brand-orange/90 transition-colors", lang === 'bn' && "font-bn normal-case text-xs")}>
          {lang === 'en' ? 'SEE FOODS' : 'খাবার দেখুন'}
        </Link>
      </div>
    </motion.div>
  );
  ...
  return (
    <div className="group bg-white rounded-[2rem] overflow-hidden border border-slate-100 hover:shadow-2xl transition-all">
      ...
      <div className="p-8">
        ...
        >
          {language === 'en' ? 'SEE FOODS' : 'খাবার দেখুন'}
        </Link>
      </div>
    </div>
  );
